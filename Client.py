...
class FlowerClient(fl.client.NumPyClient):
    ...
    def fit(self, parameters, config):
        ...
        if globalvariables.warmup:
            data = warmupclients.create_data(int(self.cid),config["server_round"], config["local_epochs"],)
            warmupclients.write_data_to_json(data,f"output{int(self.cid)}.txt")

        if self.server_round == 0 and not CheckStrategy.check_strategy() == "FedAVG":
            return [], 0, {"clientid": self.cid}
        print(f"[Client {self.cid}] fit, config: {config}")
        #set_parameters(self.net, parameters)

        if self.server_round > 0 and not globalvariables.warmup:
            set_parameters((self.net), torch.load("weightsglobal.pth"))
    ...
    def evaluate(self, parameters, config):
        ...
        #filename = f'Client_{self.cid}_DIR_0_NRC_10_E_1_K_100_R_0.csv'
        #resultsFEDNIP.write_to_csv(filename,getserverround.get_server_round('globalvariables.py'),round(accuracy, 4))

        if globalvariables.warmup:
            warmupclients.update_accuracy(accuracy,f"output{int(self.cid)}.txt")
